...
double closestUtil(const std::vector<Point>& P, int left, int right, int depth = 0) {
    int n = right - left;
    if (n <= 3) {
        return bruteForce(std::vector<Point>(P.begin() + left, P.begin() + right), n);
    }
    ...
    if (depth < 4) {
        auto futureLeft = std::async(std::launch::async, closestUtil, std::cref(P), left, mid, depth + 1);
        dr = closestUtil(P, mid, right, depth + 1);
        dl = futureLeft.get();
    } else {
        dl = closestUtil(P, left, mid, depth + 1);
        dr = closestUtil(P, mid, right, depth + 1);
    }
